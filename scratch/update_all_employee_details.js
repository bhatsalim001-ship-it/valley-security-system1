...
async function main() {
  ...
  for (const userEmp of userList) {
    const normUser = normalizeName(userEmp.name);
    
    // Find matching employee in database
    const matched = employees.find(emp => {
      const normDB = normalizeName(emp.name);
      return normDB === normUser || normDB.includes(normUser) || normUser.includes(normDB);
    });

    if (matched) {
      const updatedData = { ...matched.data };
      
      // Update details
      updatedData.fatherName = userEmp.parentage;
      updatedData.currentAddress = userEmp.address;
      updatedData.permanentAddress = userEmp.address;
      updatedData.designation = userEmp.designation;

      await client.query("UPDATE employees SET data = $1 WHERE id = $2", [JSON.stringify(updatedData), matched.id]);
      console.log(`✅ Updated: "${matched.name}" (${matched.id}) -> Father: "${userEmp.parentage}", Desig: "${userEmp.designation}"`);
      updateCount++;
    } else {
      console.log(`⚠️ Warning: Employee "${userEmp.name}" in your list was not found in the database.`);
    }
  }
  ...
  console.log(`=============================================================`);
  console.log(`Total Employees Updated: ${updateCount} out of ${userList.length}`);
  console.log(`=============================================================`);
